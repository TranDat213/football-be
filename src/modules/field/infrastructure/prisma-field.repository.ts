...
import { FieldCategory, FieldStatus, FootballField, PrismaClient, User } from "@prisma/client";
import { FieldDto, UpdateFieldDto } from "../dto/field.dto";

export class PrismaFieldRepository implements IFieldRepository {
    constructor(private readonly prisma: PrismaClient) { }

    async createField(ownerId: string, data: FieldDto, slug: string): Promise<FootballField> {
        return await this.prisma.footballField.create({
            data: {
                ...
                openTime: data.open_time ? new Date(`1970-01-01T${data.open_time}:00Z`) : null,
                closeTime: data.close_time ? new Date(`1970-01-01T${data.close_time}:00Z`) : null,
    ...
    async findByOwnerId(ownerId: string): Promise<FootballField[] | null> {
    ...
    async updateField(fieldId: string, data: UpdateFieldDto, slug?: string): Promise<FootballField> {
        return await this.prisma.footballField.update({
            ...
            data: {
                ...
                latitude: data.latitude,
                longitude: data.longitude,
                openTime: data.open_time ? new Date(`1970-01-01T${data.open_time}:00Z`) : undefined,
                closeTime: data.close_time ? new Date(`1970-01-01T${data.close_time}:00Z`) : undefined,
                status: FieldStatus.PENDING,
                slug: slug,
                updatedAt: new Date()
            }
