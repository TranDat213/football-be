...
export class CasualMatchController {
  ...
  async getParticipations(req: Request, res: Response, _next: NextFunction) {
    const userId = req.user!.id;
    const page = parseInt(req.query.page as string) || 1;
    const limit = Math.min(parseInt(req.query.limit as string) || 10, 50);
    const joinStatus = req.query.joinStatus as any;
    const paymentStatus = req.query.paymentStatus as any;
    const date = req.query.date as string | undefined;

    const result = await this.service.getParticipations(userId, { joinStatus, paymentStatus, date, page, limit });
    return res.status(200).json({ message: 'Lịch sử tham gia', ...result });
  }
  ...
  async join(req: Request, res: Response, _next: NextFunction) {
    ...
    const result = await this.service.join(req.params.id as string, userId, data, ip);
    return res.status(201).json({ message: 'Đăng ký thành công', data: result });
  }

  // POST /casual-matches/:id/payment  (backward compat)
  ...
  async getParticipants(req: Request, res: Response, _next: NextFunction) {
    const requesterId = req.user!.id;
    const result = await this.service.getMatchParticipants(req.params.id as string, requesterId);
    return res.status(200).json({ message: 'Danh sách người tham gia', data: result });
  }
}
